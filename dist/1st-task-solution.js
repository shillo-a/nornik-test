"use strict";
class Library {
    constructor(_books = [], _idCounter = 1) {
        this._books = _books;
        this._idCounter = _idCounter;
    }
    addBook({ name, author_name, publishing_year }) {
        const newBook = {
            id: this._idCounter++,
            name,
            author_name,
    ...
    updateBook(id, { name, author_name, publishing_year }) {
        const currentBook = this._books.find(book => book.id === id);
        if (!currentBook) {
            throw new Error('Книга по указаному id отсутствует');
        }
        const updatedBook = Object.assign(Object.assign({}, currentBook), { name: name ? name : currentBook.name, author_name: author_name ? author_name : currentBook.author_name, publishing_year: publishing_year ? publishing_year : currentBook.publishing_year });
        this._books = [
    ...
    getAllBooksTotalPageNum(limit) {
        const totalPageNum = Math.ceil(this._books.length / limit);
        return totalPageNum;
    }
    getAllBooksFromPage(pageNum, limit) {
        // По умолчанию сортируем по id
        const sortedBooks = [...this._books].sort((a, b) => a.id - b.id);
